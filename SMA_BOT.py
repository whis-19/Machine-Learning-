...
if __name__ == '__main__':
    ...
    while True:
        ...
        # trading
        if direction == "buy":
            map(close_order, map(lambda pos: pos.ticket, filter(lambda pos: pos.type == 1, mt5.positions_get())))

            if not mt5.positions_total():
                market_order(SYMBOL, VOLUME, direction)

        elif direction == "sell":
            map(close_order, map(lambda pos: pos.ticket, filter(lambda pos: pos.type == 0, mt5.positions_get())))
